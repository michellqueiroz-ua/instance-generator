...
def distance(place_name, df_dist, df_loc):
    ...
    for idxs, row in df_dist.iterrows():
        ...
        latitude = float(r1['latitude'].values[0])
...
def ratio_eta_real_time(place_name, df_ratio):
    ...
    #ax = ratios_pd.hist(bins=30, figsize=(15,5))
    plt.hist(ratios, density=False, bins=30)  
    plt.ylabel('count')
    plt.xlabel('ratio');
    plt.savefig('ratio.png')

    #ax.set_yscale('log')
    ##ax.set_xlabel("ratio real vs eta")
    #ax.set_ylabel("count")
    #plt.savefig('ratio.png')
    plt.close()
        ...
        for idx2, row2 in inst1.iterrows():
            ...
            if idx2 != idx1:
                ...
                if (latest_arrival2 >= row1[earliest_departure] - time_gap) and (latest_arrival2 <= row1[earliest_departure] + time_gap):
                    ...
                    #estination_point = (latitude, longitude)
                    #node_destination = ox.get_nearest_node(inst.network.G_drive, destination_point)
...
def real_data_tests_austin_database(ed, ld):
    ...
    for day in range(1,2):
        for day2 in range(1,3):
            if day2 > day:
                ...
                df_1 = pd.read_sql_query('SELECT Checkout_Date, pu_time_sec, Checkout_Kiosk_ID, Return_Kiosk_ID, Trip_Duration_Seconds, osmid_origin, osmid_destination  \
                        FROM table_record', austin_database)

                df_2 = pd.read_sql_query('SELECT Checkout_Date, pu_time_sec, Checkout_Kiosk_ID, Return_Kiosk_ID, Trip_Duration_Seconds, osmid_origin, osmid_destination \
                        FROM table_record', austin_database)
    ...
    df_gd = pd.read_sql_query('SELECT Checkout_Date, pu_time_sec, Checkout_Kiosk_ID, Return_Kiosk_ID, Trip_Duration_Seconds, osmid_origin, osmid_destination  \
                        FROM table_record', austin_database)
    ...
    for day in range(1,2):
        ...
        geographic_dispersion("Austin, Texas", df_gd_d, df_loc, day)

    df_dyn = pd.read_sql_query('SELECT Checkout_Date, pu_time_sec, Checkout_Kiosk_ID, Return_Kiosk_ID, Trip_Duration_Seconds, osmid_origin, osmid_destination \
                        FROM table_record', austin_database)
    ...
    #heatmap
    heatmap_osmnx("Austin, Texas", austin_database)

    #fitting
    df_fit = pd.read_sql_query('SELECT pu_time_sec, trip_distance FROM table_record', austin_database)
    dists = df_fit["trip_distance"].values
