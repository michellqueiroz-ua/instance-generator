...
def retrieve_network(
	...
	network = Network(G_drive, shortest_path_drive, G_walk, shortest_path_walk, bus_stops, zones, schools, vehicle_speed)
...
def generate_instance(
	...
	if pdf == "normal":
		...
		dnd = RequestDistribution(mean, std, number_of_requests, pdf, num_origins, num_destinations, which_sampled_time, is_random_origin_zones, is_random_destination_zones, origin_zones_list, destination_zones_list)
	...
	if pdf == "uniform":
		...
		dnd = RequestDistribution(min_time, max_time, number_of_requests, pdf, num_origins, num_destinations, which_sampled_time, is_random_origin_zones, is_random_destination_zones, origin_zones_list, destination_zones_list)
		request_demand.append(dnd)
	...
	if problem_type=="ODBRP":
		for replicate_num in range(num_replicates):
			generate_requests_ODBRP(network, request_demand, min_early_departure, max_early_departure, min_walk_speed, max_walk_speed, max_walking, bus_factor, replicate_num, save_dir_json, output_folder_base)
	
	if problem_type=="SBRP":

		#select which school is the destination
		school = network.schools.loc[0]
		for replicate_num in range(num_replicates):
			generate_requests_ODBRP(network, school, request_demand, min_early_departure, max_early_departure, min_walk_speed, max_walk_speed, max_walking, bus_factor, replicate_num, save_dir_json, output_folder_base)
...
if __name__ == '__main__':
	...
	if is_request_generation:
		generate_instance(place_name='Rennes, France', pdf="uniform", max_time=8, min_time=10, number_of_requests=100, num_replicates=1, min_early_departure=7, max_early_departure=11, problem_type="ODBRP", min_walk_speed=4/3.6, max_walk_speed=5/3.6, max_walking=600, vehicle_factor=2, time_unit="h")
